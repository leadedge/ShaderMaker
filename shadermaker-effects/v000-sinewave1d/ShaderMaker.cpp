...
#define FFPARAM_MOUSEX      (21)
#define FFPARAM_MOUSEY      (22)
#define FFPARAM_MOUSELEFTX  (23)
#define FFPARAM_MOUSELEFTY  (24)
#define FFPARAM_RED         (25)
#define FFPARAM_GREEN       (26)
#define FFPARAM_BLUE        (27)
#define FFPARAM_ALPHA       (28)


#define FFPARAM_SINE_SPEED1       (1)
#define FFPARAM_SINE_SPEED2       (4)
#define FFPARAM_SINE_SPEED3       (7)

#define FFPARAM_SINE_WAVELENGTH1       (2)
#define FFPARAM_SINE_WAVELENGTH2       (5)
#define FFPARAM_SINE_WAVELENGTH3       (8)

#define FFPARAM_SINE_AMPLITUDE1       (3)
#define FFPARAM_SINE_AMPLITUDE2       (6)
#define FFPARAM_SINE_AMPLITUDE3       (9)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"SM00",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	...
	"SpackOMat - Sine Wave 1d - Version 1.006\n Create 1 Dimensional sine waves",			// *** About - use your own name and details
	0,NULL
);
...
// spack o mat sine 
// as always 3 make a good couple :D
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	float val = sin(iTime + times.x*10.0 + uv.x*wavelengths.x*100.)*amplitudes.x;

	val += sin(iTime+times.y*10.0 + uv.x*wavelengths.y*100.)*amplitudes.y;
	val += sin(iTime + times.z*10.0 + uv.x*wavelengths.z*100.)*amplitudes.z;
	val /= 3.;
...
/*	SetParamInfo(FFPARAM_MOUSEX,        "X mouse",       FF_TYPE_STANDARD, 0.5f); m_UserMouseX = 0.5f;
	...
	*/
	...
	SetParamInfo(FFPARAM_SINE_SPEED1, "Frequency 1", FF_TYPE_STANDARD, 1.0f); m_UserSpeed1 = .50f;
	SetParamInfo(FFPARAM_SINE_SPEED2, "Frequency 2", FF_TYPE_STANDARD, 1.0f); m_UserSpeed2 = .50f;
	SetParamInfo(FFPARAM_SINE_SPEED3, "Frequency 3", FF_TYPE_STANDARD, 1.0f); m_UserSpeed3 = .50f;


	SetParamInfo(FFPARAM_SINE_WAVELENGTH1, "Wavelength 1", FF_TYPE_STANDARD, 1.0f); m_UserWavelength1 = .50f;
	SetParamInfo(FFPARAM_SINE_WAVELENGTH2, "Wavelength 2", FF_TYPE_STANDARD, 1.0f); m_UserWavelength2 = .50f;
	SetParamInfo(FFPARAM_SINE_WAVELENGTH3, "Wavelength 3", FF_TYPE_STANDARD, 1.0f); m_UserWavelength3 = .50f;


	SetParamInfo(FFPARAM_SINE_AMPLITUDE1, "Amplitude 1", FF_TYPE_STANDARD, 1.0f); m_UserAmplitude1 = .50f;
	SetParamInfo(FFPARAM_SINE_AMPLITUDE2, "Amplitude 2", FF_TYPE_STANDARD, 1.0f); m_UserAmplitude1 = .50f;
	SetParamInfo(FFPARAM_SINE_AMPLITUDE3, "Amplitude 3", FF_TYPE_STANDARD, 1.0f); m_UserAmplitude1 = .50f;
...
FFResult ShaderMaker::ProcessOpenGL(ProcessOpenGLStruct *pGL)
{
	...
	if(bInitialized) {
		...
		*/
		...
		// ----------------------------------------------------------
		// spack o mat sine params
		// ----------------------------------------------------------


		if (m_amplitudeLocation >= 0)
			m_extensions.glUniform3fARB(m_amplitudeLocation, m_UserAmplitude1, m_UserAmplitude2, m_UserAmplitude3);
		if (m_wavelengthLocation >= 0)
			m_extensions.glUniform3fARB(m_wavelengthLocation, m_UserWavelength1, m_UserWavelength2, m_UserWavelength3);
		if (m_frequencyLocation >= 0)
			m_extensions.glUniform3fARB(m_frequencyLocation, m_UserSpeed1, m_UserSpeed2, m_UserSpeed3);
		if (m_timesLocation >= 0)
			m_extensions.glUniform3fARB(m_timesLocation, m_userTime1, m_userTime2, m_userTime3);
...
char * ShaderMaker::GetParameterDisplay(DWORD dwIndex) {
	...
	switch (dwIndex) {
		...
		case FFPARAM_SPEED:
			cross_secure_sprintf(m_DisplayValue, 16, "xxx %d", (int)(m_UserSpeed*100.0));
			return m_DisplayValue;
	
		case FFPARAM_MOUSEX:
			cross_secure_sprintf(m_DisplayValue, 16, "xxx %d", (int)(m_UserMouseX*m_vpWidth));
			return m_DisplayValue;

		case FFPARAM_MOUSEY:
			cross_secure_sprintf(m_DisplayValue, 16, "xxx %d", (int)(m_UserMouseY*m_vpHeight));
			return m_DisplayValue;

		case FFPARAM_MOUSELEFTX:
			cross_secure_sprintf(m_DisplayValue, 16, "xxx %d", (int)(m_UserMouseLeftX*m_vpWidth));
			return m_DisplayValue;

		case FFPARAM_MOUSELEFTY:
			cross_secure_sprintf(m_DisplayValue, 16, "xxx %d", (int)(m_UserMouseLeftY*m_vpHeight));
			return m_DisplayValue;

		case FFPARAM_RED:
			cross_secure_sprintf(m_DisplayValue, 16, "xxx %d", (int)(m_UserRed*256.0));
			return m_DisplayValue;

		case FFPARAM_GREEN:
			cross_secure_sprintf(m_DisplayValue, 16, "xxx %d", (int)(m_UserGreen*256.0));
			return m_DisplayValue;

		case FFPARAM_BLUE:
			cross_secure_sprintf(m_DisplayValue, 16, "xxx %d", (int)(m_UserBlue*256.0));
			return m_DisplayValue;

		case FFPARAM_ALPHA:
			cross_secure_sprintf(m_DisplayValue, 16, "xxx %d", (int)(m_UserAlpha*256.0));
			return m_DisplayValue;
...
float ShaderMaker::GetFloatParameter(unsigned int index)
{
	switch (index) {
		...
		case FFPARAM_SINE_AMPLITUDE1:
			return m_UserAmplitude1;

		case FFPARAM_SINE_AMPLITUDE2:
			return m_UserAmplitude2;

		case FFPARAM_SINE_AMPLITUDE3:
			return m_UserAmplitude3;

		case FFPARAM_SINE_SPEED1:
			return m_UserSpeed1;

		case FFPARAM_SINE_SPEED2:
			return m_UserSpeed2;

		case FFPARAM_SINE_SPEED3:
			return m_UserSpeed3;

		case FFPARAM_SINE_WAVELENGTH1:
			return m_UserWavelength1;

		case FFPARAM_SINE_WAVELENGTH2:
			return m_UserWavelength2;

		case FFPARAM_SINE_WAVELENGTH3:
			return m_UserWavelength3;
...
FFResult ShaderMaker::SetFloatParameter(unsigned int index, float value)
{
		switch (index) {
			...
			case FFPARAM_SINE_AMPLITUDE1:
				m_UserAmplitude1 = value;
				break;

			case FFPARAM_SINE_AMPLITUDE2:
				m_UserAmplitude2 = value;
				break;

			case FFPARAM_SINE_AMPLITUDE3:
				m_UserAmplitude3 = value;
				break;
			...
			case FFPARAM_SINE_WAVELENGTH1:
				m_UserWavelength1 = value;
				break;

			case FFPARAM_SINE_WAVELENGTH2:
				m_UserWavelength2 = value;
				break;

			case FFPARAM_SINE_WAVELENGTH3:
				m_UserWavelength3 = value;
				break;
		...
		if (!m_shader.Compile(vertexShaderCode, shaderString.c_str())) {
			...
			if (!success) {
				...
				*/
				...
				// spack o mat sine controlls 
					 m_frequencyLocation = m_shader.FindUniform("frequencies");
					 m_amplitudeLocation = m_shader.FindUniform("amplitudes");
					 m_wavelengthLocation = m_shader.FindUniform("wavelengths");
					 m_timesLocation = m_shader.FindUniform("times");
