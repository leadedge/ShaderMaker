...
#define FFPARAM_SINEWAVE1_WAVELENGTH     (0) 
#define FFPARAM_SINEWAVE1_AMPLITUDE     (1) 
#define FFPARAM_SINEWAVE1_FREQUENCY     (2) 
 
#define FFPARAM_SINEWAVE2_WAVELENGTH     (3) 
#define FFPARAM_SINEWAVE2_AMPLITUDE     (4) 
#define FFPARAM_SINEWAVE2_FREQUENCY     (5) 
 
#define FFPARAM_SINEWAVE3_WAVELENGTH     (6) 
#define FFPARAM_SINEWAVE3_AMPLITUDE     (7) 
#define FFPARAM_SINEWAVE3_FREQUENCY     (8) 
 

#define FFPARAM_RESYNC     (9) 
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"SOM-SIN1D",								// Plugin unique ID											
	"sOm-SineWave1D",					// Plugin name											
	1,						   			// API major version number 													
	000,								  // API minor version number	
	1,										// Plugin major version number
	000,									// Plugin minor version number
	FF_SOURCE,						// Plugin type
	"sOm-SineWave1D",	// Plugin description
	"(c) by VJSpack-O-Mat aka Christian Kleinhuis aug2014 - 1d Multi Sine Waves. " // About
);
...
"return (sin(x*sineWave1.z +sineWave1.x)*sineWave1.y+"
"		sin(x*sineWave2.z +sineWave2.x)*sineWave2.y+"
"		sin(x*sineWave3.z +sineWave3.x)*sineWave3.y)/3;"
"}"
...
"   vec4 col_out = 0;"
"   if(evaluate(texCoord.x*PI)>texCoord.y)col_out=1;"    
 ...
 m_maxCoordsLocation(-1)
{
	...
	SetParamInfo(FFPARAM_SINEWAVE1_AMPLITUDE, "Amplitude1", FF_TYPE_STANDARD, 1.0f); 
	SetParamInfo(FFPARAM_SINEWAVE1_WAVELENGTH, "Wavelength1", FF_TYPE_STANDARD, 1.0f); 
	SetParamInfo(FFPARAM_SINEWAVE1_FREQUENCY, "Frequency1", FF_TYPE_STANDARD, 1.0f); 
	SetParamInfo(FFPARAM_SINEWAVE2_AMPLITUDE, "Amplitude2", FF_TYPE_STANDARD, 1.0f); 
	SetParamInfo(FFPARAM_SINEWAVE2_WAVELENGTH, "Wavelength2", FF_TYPE_STANDARD, 1.0f); 
	...
	SetParamInfo(FFPARAM_SINEWAVE3_WAVELENGTH, "Wavelength3", FF_TYPE_STANDARD, 1.0f); 
	...
	sineWave1.amplitude=.5;
	sineWave1.frequency=1;
	sineWave1.wavelength=1;
	sineWave1.count=0;
	
	sineWave2.amplitude=1;
	sineWave2.frequency=21;
	sineWave2.wavelength=1;
	sineWave2.count=0;

	sineWave3.amplitude=.252;
	sineWave3.frequency=1;
	sineWave3.wavelength=-4;
	sineWave3.count=0;
...
DWORD FFGLHeat::InitGL(const FFGLViewportStruct *vp)
{
  ...
  m_sinewave1Location=m_shader.FindUniform("sineWave1");
  m_sinewave2Location=m_shader.FindUniform("sineWave2");
  m_sinewave3Location=m_shader.FindUniform("sineWave3");
...
double FFGLHeat::GetTimeInSecondsSinceStart(){
   ...
   sineWave1.count+=delta*(sineWave1.wavelength*10.0f-5.0f);
   sineWave2.count+=delta*(sineWave2.wavelength*10.0f-5.0f);
   sineWave3.count+=delta*(sineWave3.wavelength*10.0f-5.0f);
...
DWORD FFGLHeat::ProcessOpenGL(ProcessOpenGLStruct *pGL)
{
  ...
  m_extensions.glUniform3fARB(m_sinewave1Location, sineWave1.count,sineWave1.amplitude,sineWave1.frequency*PI*10);
  m_extensions.glUniform3fARB(m_sinewave2Location,sineWave2.count,sineWave2.amplitude,sineWave2.frequency*PI*10);
  m_extensions.glUniform3fARB(m_sinewave3Location,sineWave3.count,sineWave3.amplitude,sineWave3.frequency*PI*10);
...
DWORD FFGLHeat::GetParameter(DWORD dwIndex)
{
	...
	case FFPARAM_SINEWAVE1_AMPLITUDE:
		*((float *)(unsigned)&dwRet) = sineWave1.amplitude;
		return dwRet;  
	case FFPARAM_SINEWAVE1_WAVELENGTH:
		*((float *)(unsigned)&dwRet) = sineWave1.wavelength;
		return dwRet;  
	case FFPARAM_SINEWAVE1_FREQUENCY:
		*((float *)(unsigned)&dwRet) = sineWave1.frequency;
		return dwRet;  
	default:
		return FF_FAIL;
	}
...
DWORD FFGLHeat::SetParameter(const SetParameterStruct* pParam)
{
	if (pParam != NULL) {
		...
		case FFPARAM_SINEWAVE1_AMPLITUDE:
			sineWave1.amplitude= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
		case FFPARAM_SINEWAVE1_FREQUENCY:
			sineWave1.frequency= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
		case FFPARAM_SINEWAVE1_WAVELENGTH:
			sineWave1.wavelength= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
	  
		case FFPARAM_SINEWAVE2_AMPLITUDE:
			sineWave2.amplitude= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
		case FFPARAM_SINEWAVE2_FREQUENCY:
			sineWave2.frequency= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
		case FFPARAM_SINEWAVE2_WAVELENGTH:
			sineWave2.wavelength= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
	  
		case FFPARAM_SINEWAVE3_AMPLITUDE:
			sineWave3.amplitude= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
		case FFPARAM_SINEWAVE3_FREQUENCY:
			sineWave3.frequency= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
		case FFPARAM_SINEWAVE3_WAVELENGTH:
			sineWave3.wavelength= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
