...
#define FFPARAM_PARAM2_X       (12)
#define FFPARAM_PARAM2_Y       (13)
#define FFPARAM_PARAM2_Z       (14)
#define FFPARAM_PARAM2_W       (15)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"X-052",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-kaliTemple",						// *** Plugin name - make it different for each plugin 
	...
	"Kali Temple", // *** Plugin description - you can expand on this
...
const vec3 LIGHT_COLOR =vec3(.85,.9,1.);
const vec3 AMBIENT_COLOR =vec3(.8,.83,1.);
const vec3 FLOOR_COLOR =vec3(1.,.7,.9);
const vec3  ENERGY_COLOR =vec3(1.,.7,.4);
const float BRIGHTNESS= .9;
const float  GAMMA =1.3;
const float SATURATION =.85;


const float detail =.00005;
...
vec2 de(vec3 pos) {
	...
	for (int i=0; i<7; i++) {//LOWERED THE ITERS
		...
		p.xz*=rot((iMouse.z/iResolution.x)*PI);
		p.xy*=rot((iMouse.w/iResolution.y)*PI);
	...
	*/
	for (int i=0; i<RAY_STEPS; i++) {
		if (d.x>det && totdist<3.0) {
			...
			if (d.x<.5 && d.x<0.03){//ONLY DOING THE GLOW WHEN IT IS CLOSE ENOUGH
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_PARAM2_X,         "Rot2 Axi2 XY",         FF_TYPE_STANDARD, 0.0f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Y,         "Rot2 Axis Z ",         FF_TYPE_STANDARD, 0.0f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Z,         "Rot2 Offset",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	
