...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S072",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-Supershape",						// *** Plugin name - make it different for each plugin 
	...
	"sOm-Supershape by mech4rhork", // *** Plugin description - you can expand on this
	"spack-O-mat Shadertoy Collection - Supershape by mech4rhork  "			// *** About - use your own name and details
);
...
"\n"
"\n"
"const vec2 renderRange = vec2(0.0, 200.0);\n"
"// parameters ( shape1, shape2 )\n"
"vec2 m  = vec2( 12., 3. );\n"
"vec2      a  = vec2( 1., -4. );\n"
"vec2      b  = vec2( 0.2, 0.2 );\n"
"vec2      n1 = vec2( -0.4, 0.3 );\n"
"vec2      n2 = vec2( 2., 1. );\n"
"vec2      n3 = vec2( 1., 1. );\n"
"vec3 rd; // used in map()\n"
"vec4 S1;\n"
...
"    float d = shape3d(p);\n"
"\tfloat s = d*.5;\n"
"\tfloat dr = (d-shape3d(p + rd*s))/s; // from eiffie\n"
"\treturn d / (1.+max(dr, 0.)); // this one too\n"
"}\n"
"float castRay( vec3 ro, vec3 rd ) {\n"
"    float startDelta=renderRange.x;\n"
"\tfloat delta=startDelta;\n"
"\tfloat stopDelta=renderRange.y;\n"
"    float maxDist = 0.00002;\n"
"    for(int i = 0; i < 128; i++) {\n"
...
" \n"
 ...
 "  float       ps = m.x+m.y+a.x+a.y+b.x+b.y+n1.x+n1.y+n2.x+n2.y+n3.x+n3.y;"
...
"    vec3 pos = ro + rd * dist;\n"
"    \n"
"    if(dist > renderRange.y) return vec4(1,0,1,0); // background\n"
"    else {\n"
"        vec3 lig = normalize(vec3(-2,3.5,-2.6));\n"
"          vec3   nor = calcNormal(pos);\n"
"          vec3   ref = reflect(rd, nor);\n"
"        float dif = clamp(dot(nor, lig), 0., 1.);\n"
"             float spe = pow(clamp(dot(reflect(-lig, nor), -rd), 0., 1.), 25.);\n"
"           float   fre = pow(clamp(1.0 + dot(nor, rd), 0., 1.), 5.);\n"
"            float  dom = smoothstep(-0.15, 0.15, ref.y);\n"
"           float   amb = 1.0;\n"
"           float   occ = calcAO(pos, nor);\n"
"float              sss = calcSSS(pos, lig);\n"
...
"vec4 params( float t ){\n"
"\tt=mod(t,7.0);\n"
"\tif(t<1.0)return vec4(1., 1.5, 1.6, 2.5);\n"
"\tif(t<2.0)return vec4(3., 2.1, 1.4, 1.3);\n"
"    if(t<3.0)return vec4(6., 1., 1.2, 1.62);\n"
"    if(t<4.0)return vec4(8., .5, 1.03, 1.91);\n"
"    if(t<5.0)return vec4(11., .87, .67, 1.06);\n"
"    if(t<6.0)return vec4(12., .4, .4, 1.3);\n"
"    if(t<7.0)return vec4(7, 1.3, .9, .57);\n"
"    if(t<8.0)return vec4(-5, .95, 1.36, 1.73);\n"
"    return vec4(4., 21., 1., 1.);\n"
"}\n"
...
"    if(t<8.0)return vec2(1.6, 2.5);\n"
"    return vec2(1.64, 1.1);\n"
"}\n"
"\n"
...
"    \n"
"\n"
"    m=(iParam2.xy+1.0)*20.0;\n"
"    n1=(iParam2.zw+1.0)*10.0;\n"
"    n2=(iParam3.xy+1.0)*10.0;\n"
"    n3=(iParam3.zw+1.0)*10.0;\n"
"    a=iParam4.xy+1.0;\n"
"    b=iParam4.zw+1.0;\n"
"\n"
"    // 2d shape 1\n"
...
"    float camDist = 25.*(iMouse.z/iResolution.x);\n"
"    vec2 drot = vec2(mo.x*PI, mo.y*PI);\n"
"    vec3 camPos = rotate(vec3(-camDist, 0, -camDist), vec3(drot.x, drot.y, 0));\n"
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Rotate Z",       FF_TYPE_STANDARD, 0.0f); m_UserMouseX = 0.0f;
	SetParamInfo(FFPARAM_MOUSEY,        "Rotate Y",       FF_TYPE_STANDARD, 0.0f); m_UserMouseY = 0.0f;
	SetParamInfo(FFPARAM_MOUSELEFTX,    "Scale",  FF_TYPE_STANDARD, 0.0f); m_UserMouseLeftX = 0.0f;
	...
	*/
	SetParamInfo(FFPARAM_PARAM2_X,         "m1",         FF_TYPE_STANDARD, 0.0f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Y,         "m2",         FF_TYPE_STANDARD, 0.0f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Z,         "n1 1",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	
SetParamInfo(FFPARAM_PARAM2_W,         "n1 2",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	

SetParamInfo(FFPARAM_PARAM3_X,         "n2 1",         FF_TYPE_STANDARD, 0.0f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM3_Y,         "n2 2",         FF_TYPE_STANDARD, 0.0f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM3_Z,         "n3 1",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	
SetParamInfo(FFPARAM_PARAM3_W,         "n3 2",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	
	

	
	SetParamInfo(FFPARAM_PARAM4_X,         "a 1",         FF_TYPE_STANDARD, 0.750f); m_UserParam4x = 0.0f;
	SetParamInfo(FFPARAM_PARAM4_Y,         "a 2",         FF_TYPE_STANDARD, 0.250f); m_UserParam4y = 0.0f;
	SetParamInfo(FFPARAM_PARAM4_Z,         "b 1",         FF_TYPE_STANDARD, 0.750f); m_UserParam4z = 0.0f;	
	SetParamInfo(FFPARAM_PARAM4_W,         "b 2",         FF_TYPE_STANDARD, 0.75f); m_UserParam4w = 0.75f;
