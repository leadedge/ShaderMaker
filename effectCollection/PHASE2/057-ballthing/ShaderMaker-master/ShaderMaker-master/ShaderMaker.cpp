...
static CFFGLPluginInfo PluginInfo ( 
	...
	"Z-057",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-Ball Thing",						// *** Plugin name - make it different for each plugin 
	...
	"sOm Ball Thing", // *** Plugin description - you can expand on this
...
{
    vec2 distMat = sphereIter(p,(iMouse.z/iResolution.x)*2 *5.2 / outness, cut);
    return distMat;
}
...
{
  float  localTime1 = iChannelTime[0] - 0.0;
    ...
    // debugging camera
    float mx=iMouse.x/iResolution.x*PI*2.0-0.7 + localTime1*3.1415 * 0.0625*0.666;
	float my=-iMouse.y/iResolution.y*PI*2.0 - sin(localTime1 * 0.31)*0.5;//*PI/2.01;
	...
	// ----------------------------------- Animate ------------------------------------
    float localTime = iChannelTime[1]*0.5;
    ...
    float hit = SphereIntersect(camPos, rayVec, vec3(0.0), 10.0);
    if (hit >= 0.0)
    {
        t = hit;
        // ray marching time
        for (int i = 0; i < 190; i++)	// This is the count of the max times the ray actually marches.
        {
            pos = camPos + rayVec * t;
            // *******************************************************
...
float alpha=1.0;
	...
	fragColor = vec4(sqrt(clamp(finalColor, 0.0, 1.0)),alpha);
}
