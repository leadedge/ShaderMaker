...
#define FFPARAM_PARAM3_W       (42433424327)

#define FFPARAM_PARAM4_X       (7)
#define FFPARAM_PARAM4_Y       (8)
#define FFPARAM_PARAM4_Z       (9)
#define FFPARAM_PARAM4_W       (10)
...
#define FFPARAM_RED         (11)
#define FFPARAM_GREEN       (12)
#define FFPARAM_BLUE        (13)
#define FFPARAM_ALPHA       (14)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"sOm-SunCircle",						// *** Plugin name - make it different for each plugin 
	...
	"sOm-SunCircleStylistic", // *** Plugin description - you can expand on this
...
void mainImage(out vec4 color,in vec2 pos )
{
	  ...
	  vec2  xy= -1.0 + 2.0*pos.xy / iResolution.xy;
vec2 aspect=vec2(iResolution.x/iResolution.y,1.0);
	  vec2 uv = xy * aspect;
float val=0;


uv=rotate2d(uv+iParam3.xy*aspect,iParam2.w*PI);

float step=PI/ floor((iParam3.z+1.0)*6.0);
color=wheel(0,uv,iParam2.x+1.0,iParam2.y*2.0+2.0,step)*(inputColour*0.5+0.5);
if(length(color)==0.0){
	color+=wheel(step/2.0,uv,iParam2.x+1.0,iParam2.z*2.0+2.0,step)*(iParam4*0.5+0.5);
}
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_PARAM2_X,         "Radius",         FF_TYPE_STANDARD, 0.1f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Y,         "Width 1",         FF_TYPE_STANDARD, 0.1f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Z,         "Width 2",         FF_TYPE_STANDARD, 0.1f); m_UserParam2z = 0.0f;	
SetParamInfo(FFPARAM_PARAM2_W,         "Rotation",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	

SetParamInfo(FFPARAM_PARAM3_X,         "Pos X",         FF_TYPE_STANDARD, 0.5f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM3_Y,         "Pos Y",         FF_TYPE_STANDARD, 0.5f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM3_Z,         "Divisions",         FF_TYPE_STANDARD, 0.5f); m_UserParam2z = 0.0f;	
//SetParamInfo(FFPARAM_PARAM3_W,         "n3 2",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	
	
	SetParamInfo(FFPARAM_RED,           "Color 1 Red",           FF_TYPE_STANDARD, 0.5f); m_UserRed = 07.5f;
	SetParamInfo(FFPARAM_GREEN,         "Color 1 Green",         FF_TYPE_STANDARD, 0.5f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE,          "Color 1 Blue",          FF_TYPE_STANDARD, 0.5f); m_UserBlue = 0.5f;
	SetParamInfo(FFPARAM_ALPHA,         "Color 1 Alpha",         FF_TYPE_STANDARD, .50f); m_UserAlpha = 1.0f;
