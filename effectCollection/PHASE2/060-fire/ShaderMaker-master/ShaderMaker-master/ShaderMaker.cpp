...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S060",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-FireNoise",						// *** Plugin name - make it different for each plugin 
	...
	"sOm FireNoise", // *** Plugin description - you can expand on this
...
char *fragmentShaderCode =STRINGIFY(
        ...
        void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
            ...
            vec2 r = vec2(fbm(p + q + iChannelTime[1] - p.x - p.y), fbm(p + q - iChannelTime[2]));
