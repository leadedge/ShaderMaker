...
static CFFGLPluginInfo PluginInfo ( 
	...
	"Y-055",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-VoroTu",						// *** Plugin name - make it different for each plugin 
	...
	"sOm Voronoi Tunnel....", // *** Plugin description - you can expand on this
...
//	float fft = texture2D( iChannel0, vec2(f,0.0) ).x;
	...
	int index=int(floor(round(f*4.0)));

	return iParam2[index]*0.5+0.5;
}

float sampleMusic()
{ 
	return 			(iParam2.x+					iParam2.y+iParam2.z+iParam2.w);
}


const float  L_hi= 1613493.0;
const float L_smile= 10813998.0;
const float  L_I= 14815374.0;
const float   L_heart= 11533764.0;
const float L_S= 15793695.0;
const float L_T= 462976.0;

const float L_NONE= 0.0;
const float L_RANDOM= 1.0;
...
float triSin(float x){
	...
	return (sin(x)+sin(x*2)+sin(x*4))*.25;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) 
{
	...
	vec3 cu  = vec3(0.0, 1.0, 0.0);
		...
		cp = vec3(0.2*triSin(time*0.2), -0.02+0.2*triSin(time*0.2), 1.5);
	...
	// fake lights
	col *= pow(1.3 + smoothstep(0.6, 0.0, abs(sin(ray.z+iChannelTime[1]*2.0)))*iParam3.y, 2.0);
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_MOUSEX,        "Edginess",       FF_TYPE_STANDARD, 0.0f); m_UserMouseX = 0.0f;
/*	SetParamInfo(FFPARAM_MOUSEY,        "Rot1 Axis Z",       FF_TYPE_STANDARD, 0.0f); m_UserMouseY = 0.0f;
	...
	*/
	...
	SetParamInfo(FFPARAM_PARAM2_X,         "Circle 1 Height 1",         FF_TYPE_STANDARD, 0.5f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Y,         "Cricle 1 Height 2 ",         FF_TYPE_STANDARD, 0.75f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Z,         "Circle 2 Height 1",         FF_TYPE_STANDARD, 0.75f); m_UserParam2z = 0.0f;	
	SetParamInfo(FFPARAM_PARAM2_W,         "Circle 2 Height 2",         FF_TYPE_STANDARD, 0.5f); m_UserParam2w = 0.25f;

	SetParamInfo(FFPARAM_PARAM3_X,         "Circle 3 Height 1",         FF_TYPE_STANDARD, 0.750f); m_UserParam3x = 0.750f;
	SetParamInfo(FFPARAM_PARAM3_Y,         "circle 3 Height 2",         FF_TYPE_STANDARD, 0.750f); m_UserParam3y = 0.750f;
	SetParamInfo(FFPARAM_PARAM3_Z,         "Circle 4 Height 1",         FF_TYPE_STANDARD, 0.750f); m_UserParam3z = 0.750f;	
	SetParamInfo(FFPARAM_PARAM3_W,         "Circle 4 Height 2",         FF_TYPE_STANDARD, 0.70f); m_UserParam3w = 0.75f;
	/*
