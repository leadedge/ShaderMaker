...
#define FFPARAM_SPEEDX     (0)
#define FFPARAM_SPEEDY     (1)
#define FFPARAM_SPEEDZ  (2) 
#define FFPARAM_SCALEX  (3) 
#define FFPARAM_SCALEY	  (4)
#define FFPARAM_SCALEZ	  (5) 
#define FFPARAM_ROTX  (6) 
#define FFPARAM_ROTY	  (7)
#define FFPARAM_ROTZ	  (8) 



#define FFPARAM_TRANSLATEX  (9) 
#define FFPARAM_TRANSLATEY	  (10)
#define FFPARAM_TRANSLATEZ	  (11) 
 
#define FFPARAM_DOTSIZE  (12)
#define FFPARAM_EDGINESS  (13)
#define FFPARAM_NUMDOTS  (14)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S016",								// Plugin unique ID											
	"sOm-Starfield",					// Plugin name											
	1,						   			// API major version number 													
	000,								  // API minor version number	
	1,										// Plugin major version number
	000,									// Plugin minor version number
	FF_SOURCE,						// Plugin type
	"sOm-Starfield",	// Plugin description
...
"void main()"
"{" 
"  gl_Position =gl_ModelViewProjectionMatrix*vec4(mod(gl_Vertex.xyz+positions,10.0)-5.0,1);"
"  gl_FrontColor =vec4(gl_Color.xyz,1-gl_Position.w/4);"
"    gl_PointSize =dotsize/gl_Position.w;"
"}";
/*
...
"float l = mylength(center,edginess);" 
"return 1-clamp(sign(l-radius),0,1);"
""
"}"
"float hash( float n ) {"
...
"   vec4 col_out = c;"
"   gl_FragColor.xyzw =circle(texCoord*2.0-1.0,1)*gl_Color;"  
"}";
 ...
 m_maxCoordsLocation(-1)  
{
	...
	SetParamInfo(FFPARAM_SPEEDX, "SpeedX", FF_TYPE_STANDARD, 0.52f); 
	SetParamInfo(FFPARAM_SPEEDY, "SpeedY", FF_TYPE_STANDARD, 0.52f); 
	SetParamInfo(FFPARAM_SPEEDZ, "SpeedZ", FF_TYPE_STANDARD, 0.52f); 
	SetParamInfo(FFPARAM_SCALEX, "ScaleX", FF_TYPE_STANDARD, 0.52f); 
	SetParamInfo(FFPARAM_SCALEY, "ScaleY", FF_TYPE_STANDARD, 0.52f); 
	SetParamInfo(FFPARAM_SCALEZ, "ScaleZ", FF_TYPE_STANDARD, 0.52f); 
	...
	SetParamInfo(FFPARAM_NUMDOTS, "DotCount", FF_TYPE_STANDARD, 0.52f); 
	SetParamInfo(FFPARAM_DOTSIZE, "DotSize", FF_TYPE_STANDARD, 1.0f); 
	SetParamInfo(FFPARAM_EDGINESS, "Edginess", FF_TYPE_STANDARD, 0.25f); 
...
DWORD FFGLHeat::InitGL(const FFGLViewportStruct *vp)
{
  ...
  m_edginessLocation= m_shader.FindUniform("edginess");
...
double FFGLHeat::GetTimeInSecondsSinceStart(){
   ...
   // increase sin values
   position.x+=delta*(speeds.x*2-1.0f);
   position.y+=delta*(speeds.y*2-1.0f);
   position.z+=delta*(speeds.z*2-1.0f);
...
//  glEnable(GL_DEPTH_TEST);
 ...
 // m_extensions.glUniform1fARB(m_heatAmountLocation, 2.0*m_Heat);
  ...
  //assign the BPM  
  m_extensions.glUniform1fARB(m_dotsizeLocation,(float)(m_dotsize*25)); 
  m_extensions.glUniform1fARB(m_edginessLocation,(float)(edginess*10)); 
  //assign the BPM 
...
glBlendFunc(GL_SRC_ALPHA, GL_ONE);
   glScalef(scale.x*1,scale.y*1,scale.z*1);
    ...
    glRotatef(rotation.x*360,1,0,0);
...
DWORD FFGLHeat::SetParameter(const SetParameterStruct* pParam)
{
	if (pParam != NULL) {
		...
		case FFPARAM_SCALEX:
			scale.x= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
	 
	 
		case FFPARAM_SCALEY:
			scale.y= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
	 
	 
		case FFPARAM_SCALEZ:
			scale.z= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
		...
		case FFPARAM_EDGINESS:
			edginess= *((float *)(unsigned)&(pParam->NewParameterValue)); 
			break;
