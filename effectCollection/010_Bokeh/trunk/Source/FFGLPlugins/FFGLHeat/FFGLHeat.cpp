...
//   .... <- max length
	"S010",								// Plugin unique ID											
//   ................ <- max length
	"sOm-Bokeh",					// Plugin name											
	1,						   			// API major version number 													
	000,								  // API minor version number	
	1,										// Plugin major version number
	000,									// Plugin minor version number
	FF_EFFECT,						// Plugin type
	"sOm-Bokeh",	// Plugin description
...
"float golden_angle = (PI * (3.0-sqrt(5.0))) * ITERATIONS;"
""
"vec2 Sample(in float i)"
"{"
"    float theta = golden_angle * i;"
"    return sqrt(i+.001) * vec2(cos(theta), sin(theta));"
"}"
""
...
"for (float j = 0.0; j < 1.0; j += ONEOVER_ITR)"
"    {"
"vec3 col = texture2D(iChannel0, uv + pixel * Sample(j)).xyz;"
"vec3 bokeh = vec3(.3) + pow(col, vec3(10.0)) * amount;"
"acc += col * bokeh;"
"div += bokeh;"
"}"
"return acc / div;"
...
"   gl_FragColor = vec4(Bokeh(uv, bokehRadius ,bokehAmount), 1.0);"  
"}";
