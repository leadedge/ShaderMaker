...
#define FFPARAM_MOUSEY      (3)
#define FFPARAM_MOUSELEFTX  (9996)
#define FFPARAM_MOUSELEFTY  (7999)
#define FFPARAM_PARAM2_X       (4)
#define FFPARAM_PARAM2_Y       (44444)
#define FFPARAM_PARAM2_Z       (55555)
#define FFPARAM_PARAM2_W       (66666)


#define FFPARAM_PARAM3_X       (5)
#define FFPARAM_PARAM3_Y       (6)
#define FFPARAM_PARAM3_Z       (7)
#define FFPARAM_PARAM3_W       (8)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S070",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-Quadtree",						// *** Plugin name - make it different for each plugin 
	...
	"sOm-Quadtree by  FabriceNeyret2", // *** Plugin description - you can expand on this
...
float  P_SUBDIV= (.5*iParam2.x+0.5);
...
void mainImage( out vec4 fragColor, vec2 uv )
{
   ...
   // if (m.x+m.y<1e-2*R.x) m = R*(.5+.5*sin(.1*iGlobalTime+vec2(0,1.6)));
    ...
    float randIndex=rnd(vec3(z*u+1.,z));
    uv = z/2.-abs(uv-z*u);
    int index=int(floor(randIndex*4));
    uv*=0.10*((iParam3[index]+1.0)/2.0);
    fragColor = min(uv.x,uv.y)<1. ? vec4(0) :
    			// vec4(1); // vec4(z/R.y);
				.6+.4*cos(6.28*randIndex+vec4(0,2.1,-2.1,0));
}
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	SetParamInfo(FFPARAM_PARAM2_X,         "Prop",         FF_TYPE_STANDARD, 0.0f); m_UserParam2x = 0.0f;
...
SetParamInfo(FFPARAM_PARAM3_X,         "Scale #4",         FF_TYPE_STANDARD, 0.0f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM3_Y,         "Scale #2 ",         FF_TYPE_STANDARD, 0.0f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM3_Z,         "Scale #3",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	
	SetParamInfo(FFPARAM_PARAM3_W,         "Scale #4",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	
