...
#define FFPARAM_RED         (13)
#define FFPARAM_GREEN       (14)
#define FFPARAM_BLUE        (15)
#define FFPARAM_ALPHA       (16)

#define FFPARAM_PARAM2_X       (5)
#define FFPARAM_PARAM2_Y       (6)
#define FFPARAM_PARAM2_Z       (7)
#define FFPARAM_PARAM2_W       (8)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S053",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-Circles",						// *** Plugin name - make it different for each plugin 
	...
	"sOm Some Circles....", // *** Plugin description - you can expand on this
	"spack-O-mat Circles"			// *** About - use your own name and details
);
...
float lengthModified(vec2 x, float edginess){

return pow(pow(abs(x.x),edginess)+pow(abs(x.y),edginess),1/edginess);
...
float circle(vec2 center,float radius,float width)
{
	...
	float l=lengthModified(center,(iParam1.x+1.0)*6.0);
...
vec2 rotate2d(vec2 coord,float angle){
	...
	vec2 result;
	result.x=rot.x*coord.y-rot.y*coord.x;
	result.y=rot.x*coord.x+rot.y*coord.y;

	return result;

}
float circleSegment(vec2 center,float radius,float width,float angleStart,float angleEnd)
{
	float val=circle(center,radius,width);
	angleStart=mod(angleStart,PI*2);
	angleEnd=mod(angleEnd,PI*2);

	float angle=atan(center.x,center.y)+PI;
	if(angle<angleStart)val=0;
	if(angle>angleEnd)val=0;
...
float circleSegmentAngle(float rot,vec2 center,float radius,float width,float angleStart,float angleEnd)
{
	return circleSegment(rotate2d(center,rot),radius,width,angleStart,angleEnd);
}

float circleSegmentModulus(float rot,vec2 center,float radius,float width,float dist)
{
		float val=circle(center,radius,width);
...
float wheel(float rot,vec2 center,float radius,float width,float dist)
{
	float val=circleSegmentModulus(rot,center,radius,width,dist);
...
val+=circleSegmentModulus(iChannelTime[0]     ,uv,0.25,0.25*iParam2.x,PI/2);
val+=circleSegmentModulus(iChannelTime[0]+PI/4,uv,0.25,0.25*iParam2.y,PI/2);


val+=circleSegmentModulus(iChannelTime[1]     ,uv,0.5,0.25*iParam2.z,PI/2);
val+=circleSegmentModulus(iChannelTime[1]+PI/4,uv,0.5,0.25*iParam2.w,PI/2);


val+=circleSegmentModulus(iChannelTime[2]     ,uv,0.75,0.25*iParam3.x,PI/2);
val+=circleSegmentModulus(iChannelTime[2]+PI/4,uv,0.75,0.25*iParam3.y,PI/2);


val+=circleSegmentModulus(iChannelTime[3]     ,uv,1.,0.25*iParam3.z,PI/2);
val+=circleSegmentModulus(iChannelTime[3]+PI/4,uv,1.,0.25*iParam3.w,PI/2);





	vec3 color=inputColour;
	float alpha=0.0;
	if(val>0.0){
	alpha=1.0;
	}
	gl_FragColor = vec4( color*clamp(val,0.,1.), alpha );
}
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	...
	SetParamInfo(FFPARAM_SPEED2,         "Speed 2",         FF_TYPE_STANDARD, 0.5f); m_UserSpeed2 = 0.5f;
	SetParamInfo(FFPARAM_SPEED3,         "Speed 3",         FF_TYPE_STANDARD, 0.5f); m_UserSpeed3 = 0.5f;
	SetParamInfo(FFPARAM_SPEED4,         "Speed 4",         FF_TYPE_STANDARD, 0.5f); m_UserSpeed4 = 0.5f;

	SetParamInfo(FFPARAM_MOUSEX,        "Edginess",       FF_TYPE_STANDARD, 0.3f); m_UserMouseX = 0.3f;
/*	SetParamInfo(FFPARAM_MOUSEY,        "Rot1 Axis Z",       FF_TYPE_STANDARD, 0.0f); m_UserMouseY = 0.0f;
	...
	*/
	...
	SetParamInfo(FFPARAM_RED,           "Red",           FF_TYPE_STANDARD, 1.0f); m_UserRed = 07.5f;
	SetParamInfo(FFPARAM_GREEN,         "Green",         FF_TYPE_STANDARD, 1.0f); m_UserGreen = 0.5f;
	SetParamInfo(FFPARAM_BLUE,          "Blue",          FF_TYPE_STANDARD, 1.0f); m_UserBlue = 0.5f;
	SetParamInfo(FFPARAM_ALPHA,         "Alpha",         FF_TYPE_STANDARD, 1.0f); m_UserAlpha = 1.0f;
	
	SetParamInfo(FFPARAM_PARAM2_X,         "Circle 1 Height 1",         FF_TYPE_STANDARD, 0.55f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Y,         "Cricle 1 Height 2 ",         FF_TYPE_STANDARD, 0.6f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Z,         "Circle 2 Height 1",         FF_TYPE_STANDARD, 0.55f); m_UserParam2z = 0.0f;	
	SetParamInfo(FFPARAM_PARAM2_W,         "Circle 2 Height 2",         FF_TYPE_STANDARD, 0.6f); m_UserParam2w = 0.25f;

	SetParamInfo(FFPARAM_PARAM3_X,         "Circle 3 Height 1",         FF_TYPE_STANDARD, 0.55f); m_UserParam3x = 0.750f;
	SetParamInfo(FFPARAM_PARAM3_Y,         "circle 3 Height 2",         FF_TYPE_STANDARD, 0.6f); m_UserParam3y = 0.750f;
	SetParamInfo(FFPARAM_PARAM3_Z,         "Circle 4 Height 1",         FF_TYPE_STANDARD, 0.55f); m_UserParam3z = 0.750f;	
	SetParamInfo(FFPARAM_PARAM3_W,         "Circle 4 Height 2",         FF_TYPE_STANDARD, 0.6f); m_UserParam3w = 0.75f;
	/*
