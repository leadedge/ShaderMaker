...
#define FFPARAM_MOUSEX      (9991)
#define FFPARAM_MOUSEY      (9995)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S068",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-CloudySunset",						// *** Plugin name - make it different for each plugin 
	...
	"sOm-CloudySunset", // *** Plugin description - you can expand on this
...
{
    return pow(saturate(fbm(vec2(scalex,scaley)*(uv+vec2(speed,0)*iGlobalTime*8))-(1.0-density)), 1.0-sharpness);
}
...
vec3 render(vec2 uv)
{
    ...
    // sun
    vec2 spos = uv - vec2(iParam2.x,iParam2.y);
    float sun = exp(-((iParam2.z+1.0)*20)*dot(spos,spos));
...
//	SetParamInfo(FFPARAM_MOUSEX,        "Rose expand",       FF_TYPE_STANDARD, 0.0f); m_UserMouseX = 0.0f;
	...
	*/
	SetParamInfo(FFPARAM_PARAM2_X,         "Sun Pos X",         FF_TYPE_STANDARD, 0.0f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Y,         "Sun Pos Y",         FF_TYPE_STANDARD, 0.0f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Z,         "Sun SIze",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	
