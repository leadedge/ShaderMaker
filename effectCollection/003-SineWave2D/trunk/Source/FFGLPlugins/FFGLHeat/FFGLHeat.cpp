...
//   ................ <- max length
	...
	"sOm-Sine Plasma 2D",	// Plugin description
	"(c) by VJSpack-O-Mat aka Christian Kleinhuis - 2d Sinus Plasma. " // About
);
...
" return (sin(p.x*factX.z +factX.x)*factX.y+cos(p.y*factY.z +factY.x)*factY.y)/2;"		
"}"
"float evaluate(vec2 p){"
" return "
"      (evaluate2d(p*rot2d(angles.x),sineWave1,sineWave1Y)+"
"	   evaluate2d(p*rot2d(angles.y),sineWave2,sineWave2Y)+"
"	   evaluate2d(p*rot2d(angles.y),sineWave3,sineWave3Y))/3;"
"}"
...
"   vec4 col_out;"
"	col_out.xyz =clamp( evaluate(texCoord*PI),0,1) ;"    
"  col_out.w=1;"
