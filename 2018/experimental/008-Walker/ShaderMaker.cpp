...
#define FFPARAM_COLOR1_ALPHA       (20)  

#define FFPARAM_COLOR2_RED       (21)  
#define FFPARAM_COLOR2_GREEN       (22)  
#define FFPARAM_COLOR2_BLUE       (23)  
#define FFPARAM_COLOR2_ALPHA       (24)   
...
static CFFGLPluginInfo PluginInfo(
	...
	"SM08",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"SoM Walker",						// *** Plugin name - make it different for each plugin 
	...
	"SoM Walker",     // *** Plugin description - you can expand on this
...
char *fragmentShaderCode = STRINGIFY(
	...
	float walkPhase = inputVector1.x*2.0*PI;
float bodyPhase = inputVector1.y*2.0*PI;
float headPhase = inputVector1.z*2.0*PI;
float handPhase = inputVector1.w*2.0*PI;
float scroll1 = inputVector2.w;
float scale1 = inputVector2.z*inputVector2.z*10.0;
...
float walkingDude(vec2 uv, float time) {
	...
	//body
	vec2 bodyPos = vec2(0.0, sin(bodyPhase*8.0)*.15 + .55 - floorHeight);
	val += circle((uv - bodyPos)*vec2(1.25, 1.0), .2);


	vec2 headPos = bodyPos + vec2(cos3Big(headPhase)*0.2, 0.4 + cos3Big(headPhase*1.12)*0.2);
	val += circle(uv - headPos, .1);

	//val+=circleHalve(uv+footDist+cosey(iTime*4)*0.4,cos(PI+iTime*4)*0.05+0.1);

	vec2 footDist = vec2(0.35, 0);
	vec2 footLeft = footDist + cosey(PI / 2.0 + walkPhase + PI)*0.4;
	float footLeftRadius = cos(walkPhase + PI)*0.05 + 0.15;
	val += circleHalve(uv + footLeft, footLeftRadius);

	footDist = vec2(-0.35, 0.0);
	vec2 footRightPos = footDist + cosey(PI / 2.0 + walkPhase)*0.4;
	val += circleHalve(uv + footRightPos, cos(walkPhase)*0.05 + 0.15);
	...
	vec2 handLeft = handDist + vec2(-cos3Big(PI / 2.0 + handPhase*1.3 + PI)*0.2, cos3Big(PI / 2.0 + handPhase*1.11 + PI)*0.25);
	...
	vec2 handRight = handDist + vec2(cos3Big(PI / 2.0 + handPhase*1.3 + PI)*0.2, cos3Big(PI / 2.0 + handPhase*1.11 + PI)*0.25);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	uv.x += scroll1;
	//body
	val = walkingDudeScaled(uv, iTime, scale1);
	uv.x = uvorig.x + scroll1*.5;
...
ShaderMaker::ShaderMaker() :CFreeFrameGLPlugin()
{
	...
	SetParamInfo(FFPARAM_VECTOR1_Y, "Walk", FF_TYPE_STANDARD, 0.625f);
	SetParamInfo(FFPARAM_VECTOR1_Z, "Body", FF_TYPE_STANDARD, 0.5f);
	SetParamInfo(FFPARAM_VECTOR1_W, "Head", FF_TYPE_STANDARD, 0.5f);
...
//	SetParamInfo(FFPARAM_SPEEDS_Z, "Seed 3 Speed", FF_TYPE_STANDARD, 0.0f);
	SetParamInfo(FFPARAM_VECTOR3_Y, "Seed 3dddd Power", FF_TYPE_STANDARD, 0.625f);
