...
#define FFPARAM_SINYSPEED1	  (2)
#define FFPARAM_SINYSPEED2	  (3)
#define FFPARAM_SINXVARIANCE1  (4) 
#define FFPARAM_SINXVARIANCE2  (5) 
...
#define FFPARAM_HUESCALE  (18)
#define FFPARAM_SATURATIONSCALE  (19)
#define FFPARAM_VALUSCALE  (20)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S005",								// Plugin unique ID											
	"sOm-SineDots",					// Plugin name											
	1,						   			// API major version number 													
	000,								  // API minor version number	
	1,										// Plugin major version number
	000,									// Plugin minor version number
	FF_SOURCE,						// Plugin type
	"sOm-SineDots",	// Plugin description
...
"	return (sin(p)+sin(p*2)+sin(p*4))/3;"
"}"
"float cos3Big(float p){"
"	return (cos(p)+cos(p*2)+cos(p*4))/3;"
"}"

"void main()"
"{"
" varyingTextureCoordinate=gl_MultiTexCoord1;" 
"  gl_Position =gl_ModelViewProjectionMatrix*  "
"             vec4("
"					sin(gl_Vertex.x*sineFactors.y+sineFactors.x)*amplitudes.x+"
"					sin(gl_Vertex.x*sineFactors2.y+sineFactors2.x)*amplitudes.y"
","
"					cos(gl_Vertex.x*sineFactors.w+sineFactors.z)*amplitudes.z+"
"					cos(gl_Vertex.x*sineFactors2.w+sineFactors2.z)*amplitudes.w,"
" gl_Vertex.z,gl_Vertex.w) ;"
"  gl_TexCoord[1] = gl_MultiTexCoord1;"
"  gl_FrontColor = gl_Color;"
"gl_PointSize=dotsize*sin(gl_Vertex.x)+dotsize;"
"}";
...
"   vec4 col_out = circle(texCoord*2-1,1);" 
    "   gl_FragColor.xyzw =col_out;"   
"}";
 ...
 m_maxCoordsLocation(-1)
{
	...
	SetParamInfo(FFPARAM_SINXSPEED1, "XSpeed1", FF_TYPE_STANDARD, 0.52f);
	SetParamInfo(FFPARAM_SINXSPEED2, "XSpeed2", FF_TYPE_STANDARD, 0.252f);
	SetParamInfo(FFPARAM_SINXVARIANCE1, "XVariance1", FF_TYPE_STANDARD, 0.3f); 
	SetParamInfo(FFPARAM_SINYSPEED1, "YSpeed1", FF_TYPE_STANDARD, 0.31f);
	SetParamInfo(FFPARAM_SINYSPEED2, "YSpeed2", FF_TYPE_STANDARD, 0.6f);
	SetParamInfo(FFPARAM_SINYVARIANCE1, "YVariance", FF_TYPE_STANDARD, 0.05f); 
	SetParamInfo(FFPARAM_NUMDOTS, "DotCount(*100)", FF_TYPE_STANDARD, 0.1f);
	...
	SetParamInfo(FFPARAM_DOTSIZE, "Dotsize", FF_TYPE_STANDARD, .10f);
	SetParamInfo(FFPARAM_AMPLITUDE1, "AmplitudeX1", FF_TYPE_STANDARD, 0.5f);
	SetParamInfo(FFPARAM_AMPLITUDE2, "AmplitudeX2", FF_TYPE_STANDARD, 0.5f);
	SetParamInfo(FFPARAM_AMPLITUDE3, "AmplitudeY1", FF_TYPE_STANDARD, 0.5f);
	SetParamInfo(FFPARAM_AMPLITUDE4, "AmplitudeY2", FF_TYPE_STANDARD, 0.5f);
	SetParamInfo(FFPARAM_SINXVARIANCE2, "XVariance2", FF_TYPE_STANDARD, 0.4f);
	SetParamInfo(FFPARAM_SINYVARIANCE2, "YVariance2", FF_TYPE_STANDARD, 0.1f);
	SetParamInfo(FFPARAM_ROUNDS, "Rounds(*10)", FF_TYPE_STANDARD, 0.15f);
	...
	m_sinXVariance=0.3;
		m_sinXVarianceCurrent=0.3;
	m_sinXVariance2=0.4;
	m_sinXVariance2Current=0.4;
	...
	m_sinYVariance=0.025f;
	m_sinYVariance2=.01;
	m_sinYVarianceCurrent=0.025f;
	m_sinYVariance2Current=0.01;
 ...
 int numDots=10000;
  for(int i=0;i<numDots;i++){
	//  m_extensions.glMultiTexCoord2f(GL_TEXTURE1, 0,0);
	  glVertex2f(
		  ((float)i/(float)(numDots/2))*112*PI,-1);
  }
  glEnd();
...
double FFGLHeat::GetTimeInSecondsSinceStart(){
   ...
   sinCounter2+=delta*(m_sinYSpeed*4-2);
   
   sinCounter3+=delta*(m_sinXSpeed2*4-2);
   sinCounter4+=delta*(m_sinYSpeed2*4-2);
   
   m_SystemTimeLastFrame=st;

   
   m_sinXVarianceCurrent=lerp(m_sinXVarianceCurrent,m_sinXVariance,0.01);
   m_sinXVariance2Current=lerp(m_sinXVariance2Current,m_sinXVariance2,0.01);

   m_sinYVarianceCurrent=lerp(m_sinYVarianceCurrent,m_sinYVariance,0.01);
   m_sinYVariance2Current=lerp(m_sinYVariance2Current,m_sinYVariance2,0.01);
...
glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
  ...
  m_extensions.glUniform4fARB(m_sineFactorsLocation, sinCounter1,(m_sinXVarianceCurrent),sinCounter2,((m_sinYVarianceCurrent) ));
  m_extensions.glUniform4fARB(m_sineFactors2Location, sinCounter3,(m_sinXVariance2Current),sinCounter4,((m_sinYVariance2Current) ));
  m_extensions.glUniform4fARB(m_amplitudesLocation, m_amplitudeX1,m_amplitudeX2,m_amplitudeY1,m_amplitudeY2);
 ...
 // m_extensions.glUniform1fARB(m_heatAmountLocation, 2.0*m_Heat);
  ...
  //assign the BPM  
  m_extensions.glUniform1fARB(m_dotsizeLocation,(float)(m_dotsize*100));
