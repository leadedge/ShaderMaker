...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S061",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-Simplicity",						// *** Plugin name - make it different for each plugin 
	...
	"sOm Simplicity", // *** Plugin description - you can expand on this
...
float field(in vec3 p) {
	...
	for (int i = 0; i < 16; ++i) {
		float mag = dot(p, p);
		p = abs(p) / mag + vec3(-.5, -.4, -.8  + (-0.13 * sin(iGlobalTime * .1) - .4));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 rayPos = vec3(120.0, 2.0 * sin(iGlobalTime / 2.7), -12.0);
		
    
	rayPos.xz = rotate2d(rayPos.xz, iGlobalTime / 7.0 + mouse.x);
	rayDir.xz = rotate2d(rayDir.xz, iGlobalTime / 7.0 + mouse.y);
	...
	fragColor.rgb = color;
fragColor.w=length(fragColor.rgb);
}
	)
	;
