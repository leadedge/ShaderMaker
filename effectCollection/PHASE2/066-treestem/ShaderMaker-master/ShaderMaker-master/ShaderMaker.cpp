...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S066",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-TreeStem",						// *** Plugin name - make it different for each plugin 
	...
	"eiffie Tree Stem", // *** Plugin description - you can expand on this
...
"\n"
"\tp.x+=sin(p.y)*iParam2.z*2;\n"
"\tfloat r=length(p.xz);\n"
"\tfloat y=p.y;\n"
"\tif(r>2.75)return r-1.4;\n"
"\tfloat trunk=r-0.1;\n"
"\tp.y+=r*r*(sin(iChannelTime[1]*4)*iParam2.x+iParam2.y);\n"
"\tfloat a=atan(p.z,p.x);\n"
"\tp.y=mod(p.y+a/TAO,0.5)-.25;\n"
...
"\ta2=mod(a1,0.6)-0.1;\n"
"\tp.xz=vec2(cos(a2),sin(a2))*r2*+m.x;\n"
"\tp.x=clamp(p.x-0.4+a1*a1*0.02,0.0,0.4);\n"
"\tp.yz=vec2(abs(p.z)-p.y,abs(p.z))*0.7;\n"
"\tp=abs(p);\n"
"\tfloat frond=max(p.x,max(p.y,p.z-.025+r2*.075+a1*a1*.0025));\n"
...
"\tvec3 R=reflect(rd,N);\n"
"\tvec3 lc=vec3(1.0,0.9,0.8);\n"
"\tvec3 sc=vec3(0.5,0.9,0.4);\n"
"\tvec3 rc=Sky(R);\n"
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
	...
	*/
	SetParamInfo(FFPARAM_PARAM2_X,         "Sway Radius",         FF_TYPE_STANDARD, 0.0f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Y,         "Sway Offset ",         FF_TYPE_STANDARD, 0.0f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Z,         "Stem Wave",         FF_TYPE_STANDARD, 0.0f); m_UserParam2z = 0.0f;	
