...
#define FFPARAM_PARAM5_X       (24)
#define FFPARAM_PARAM5_Y       (25)
#define FFPARAM_PARAM5_Z       (26)
#define FFPARAM_PARAM5_W       (27)


#define FFPARAM_PARAM3_CAMERA_X       (28)
#define FFPARAM_PARAM3_CAMERA_Y       (29)
#define FFPARAM_PARAM3_CAMERA_Z       (30)

#define FFPARAM_PARAM3_CAMERA_TARGET_X       (31)
#define FFPARAM_PARAM3_CAMERA_TARGET_Y       (32)
#define FFPARAM_PARAM3_CAMERA_TARGET_Z       (33)
...
static CFFGLPluginInfo PluginInfo ( 
	...
	"S064",								// *** Plugin unique ID (4 chars) - this must be unique for each plugin
	"sOm-Lines",						// *** Plugin name - make it different for each plugin 
	...
	"sOm Lines", // *** Plugin description - you can expand on this
...
{
	float angle1=round((atan(p.y,p.x)/PI)*2);
	float angle2=round((atan(p.y,p.z)/PI)*2);




    return length(p)-(s*rand(vec2(angle2,0)));
}


float sdSphereCK( vec3 p, float s )
{
    return length(p)-s;
}
...
float sdMyThing(vec3 p,vec3 param1,vec3 param2) {
	float radius=0.1;
	float width=2.0;
if(mod((p.x+width),(width*2))>width){
	p.x=mod(p.x,2.0)-.5*width;
}else{
	p.x=-(mod(p.x,2.0)-.5*width);
}

p.z=mod(p.z,width)-0.5*width;

	return min(
			sdCapsule(p,param1,param2,radius),
			sdCapsule(p,vec3(-param1.x,param1.y,-param1.z),vec3(param2.x,param2.y,param2.z),radius)
			)
...
vec2 map( in vec3 pos )
{
    ...
    res=opU(res,
    		vec2
			(
					sdMyThing(pos,iParam2.xyz,iParam3.xyz),100.0));

    res=opU(res,
    		vec2
			(
					sdMyThing(pos+vec3(0,0,1),iParam4.xyz,iParam5.xyz),50.0));
...
vec3 pathi(float time){

	return vec3(
			triSin(time*0.01297   )*10+
			triSin(time*0.02437      )*10,
			triSin(time*0.01381   )*2+2.0,
			triCos(time*0.01453   )*10.0+
			triCos(time*0.02693     )*10.0
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	ro=pathi(iGlobalTime);
	ta=pathi(iGlobalTime+10.5);
ta.y*=0.5;
...
ShaderMaker::ShaderMaker():CFreeFrameGLPlugin()
{
 ...
 SetParamInfo(FFPARAM_MOUSELEFTX,    "Scale",  FF_TYPE_STANDARD, 0.5f); m_UserMouseLeftX = 0.0f;
	...
	SetParamInfo(FFPARAM_PARAM2_X,         "Point1 X",         FF_TYPE_STANDARD, 1.0f); m_UserParam2x = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Y,         "Point1 Y",         FF_TYPE_STANDARD, 0.5f); m_UserParam2y = 0.0f;
	SetParamInfo(FFPARAM_PARAM2_Z,         "Point1 Z",         FF_TYPE_STANDARD, 0.5f); m_UserParam2z = 0.0f;	
	SetParamInfo(FFPARAM_PARAM2_W,         "unused",         FF_TYPE_STANDARD, 0.5f); m_UserParam2w = 0.25f;

	SetParamInfo(FFPARAM_PARAM3_X,         "Point2 X",         FF_TYPE_STANDARD, 0.50f); m_UserParam3x = 0.750f;
	SetParamInfo(FFPARAM_PARAM3_Y,         "Point2 Y",         FF_TYPE_STANDARD, 0.50f); m_UserParam3y = 0.750f;
	SetParamInfo(FFPARAM_PARAM3_Z,         "Point2 Z",         FF_TYPE_STANDARD, 0.50f); m_UserParam3z = 0.750f;	
	SetParamInfo(FFPARAM_PARAM3_W,         "unused",         FF_TYPE_STANDARD, 0.50f); m_UserParam3w = 0.75f;
	
	SetParamInfo(FFPARAM_PARAM4_X,         "Point3 X",         FF_TYPE_STANDARD, 1.0f); m_UserParam4x = 0.0f;
	SetParamInfo(FFPARAM_PARAM4_Y,         "Point3 Y",         FF_TYPE_STANDARD, 0.50f); m_UserParam4y = 0.0f;
	SetParamInfo(FFPARAM_PARAM4_Z,         "Point3 Z",         FF_TYPE_STANDARD, 0.50f); m_UserParam4z = 0.0f;	
	SetParamInfo(FFPARAM_PARAM4_W,         "unused",         FF_TYPE_STANDARD, 0.5f); m_UserParam4w = 0.75f;
	 
	
	SetParamInfo(FFPARAM_PARAM5_X,         "Point4 X",         FF_TYPE_STANDARD, 0.50f); m_UserParam4x = 0.0f;
	SetParamInfo(FFPARAM_PARAM5_Y,         "Point4 Y",         FF_TYPE_STANDARD, 0.50f); m_UserParam4y = 0.0f;
	SetParamInfo(FFPARAM_PARAM5_Z,         "Point4 Z",         FF_TYPE_STANDARD, 0.50f); m_UserParam4z = 0.0f;	
	SetParamInfo(FFPARAM_PARAM5_W,         "unused",         FF_TYPE_STANDARD, 0.5f); m_UserParam4w = 0.75f;
...
char * ShaderMaker::GetParameterDisplay(DWORD dwIndex) {
	...
	case FFPARAM_PARAM3_CAMERA_TARGET_Z:
		...
		case FFPARAM_PARAM5_X:
			cross_secure_sprintf(m_DisplayValue, 16, "%d", (int)(m_UserParam5x));
			return m_DisplayValue;
		case FFPARAM_PARAM5_Y:
			cross_secure_sprintf(m_DisplayValue, 16, "%d", (int)(m_UserParam5y));
			return m_DisplayValue;
		case FFPARAM_PARAM5_Z:
			cross_secure_sprintf(m_DisplayValue, 16, "%d", (int)(m_UserParam5z));
			return m_DisplayValue;
		case FFPARAM_PARAM5_W:
			cross_secure_sprintf(m_DisplayValue, 16, "%d", (int)(m_UserParam5w));
			return m_DisplayValue;
...
case FFPARAM_PARAM4_W:
		...
		case FFPARAM_PARAM5_X:
			return m_UserParam5x;
case FFPARAM_PARAM5_Y:
			return m_UserParam5y;
case FFPARAM_PARAM5_Z:
			return m_UserParam5z;
case FFPARAM_PARAM5_W:
			return m_UserParam5w;
